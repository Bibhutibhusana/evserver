...
public interface XMLBankDetailsRepo extends JpaRepository<XMLBankDetailsModel, String>{
	...
	@Query(value="select ebd.appl_no,ebd.name, acc_no as accountNo,ed.regn_no,ebd.acc_type,  ifsc_code as ifsc, ed.mob_no as mobileNo, ed.address,"
			+ "es.sub_amnt as amount, es.op_dt,ebd.appl_no as applNo, ed.off_cd from evschema.evt_bank_details ebd "
			+ "inner join evschema.evt_details ed on ed.regn_no=ebd.regn_no "
			+ "inner join evschema.evt_status es  on ebd.appl_no = es.appl_no  where ebd.op_dt =:toDay and es.approval='y' and not exists(select * from  evschema.evt_ifms_transaction eit where eit.regn_no=ebd.regn_no ) limit 4;",nativeQuery=true)
